...
train_l_loader, val_loader, test_loader = data_loader_main.build_supervised()
...
for epoch_idx in range(args.epochs):
    ...
    for iter_idx in range(train_epoch):
        ...
        data_obj_l = train_l_dataset.next()
        imgs, segmaps = data_obj_l['img'].to(device), data_obj_l['segmap'].to(device)
    ...
    if (epoch_idx + 1)%10==0:
        ...
        c_iou, m_iou, c_acc = calculate_iou(iou_acc)
        m_iou = np.nanmean(c_iou)
